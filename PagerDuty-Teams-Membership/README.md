...
1. Write the JSON schema describing your resource, [pagerduty-teams-membership.json](./pagerduty-teams-membership.json)
2. Implement your resource handlers in [handlers.ts](./pagerduty-teams-membership/handlers.ts)

> Don't modify [models.ts](./pagerduty-teams-membership/models.ts) by hand, any modifications will be overwritten when the `generate` or `package` commands are run.
